...
$(function(){
	...
	$('.uu .pwd .iconfont').click(function(){
	...
	$('#pay a.button').click(function(){
		...
		$.post(window.location.href).then(function(data){
			if (data.status == 2) {
				...
				layer.open({
					type: 1,
					content: '<img style="width:90%;padding:0 5%;" src="/themes/default/assets/img/browser.png">',
		...
		loadDownFn: function(me){
			...
			$.post(window.location.href, {page:page}).then(function(data){
				...
				$('#user-order .countdown:not(.lock)').each(function(i,n){
					...
					$(n).addClass('lock').html(countDown(endtime));
					setInterval(function(){
						$(n).html(countDown(endtime));
					}, 1000);
				});
			}, function(){
				$.msg({msg:'通讯错误'});
