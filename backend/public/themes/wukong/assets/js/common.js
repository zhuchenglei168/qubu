...
$(function(){
	...
	$('#home .list a.button').click(function(){
		layer.open({type:2});
	...
	$('#pay .btn a.button').click(function(){
		layer.open({type:2});
		$.post(window.location.href).then(function(data){
			if (data.status == 2) {
				...
				layer.open({
					type: 1,
					content: '<img style="width:90%;padding:0 5%;" src="/themes/wukong/assets/img/browser.png">',
		...
		loadDownFn: function(me){
			...
			$.post(window.location.href, {page:page}).then(function(data){
				...
				$('#user-order .countdown:not(.lock)').each(function(i,n){
					...
					$(n).addClass('lock').html(countDown(endtime)+' 结算中');
					setInterval(function(){
						$(n).html(countDown(endtime)+' 结算中');
					}, 1000);
				});
			}, function(){
				$.msg({msg:'通讯错误'});
