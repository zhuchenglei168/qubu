...
$bodys = "{\"image\":\"".$shenghuozhao."\",\"image_type\":\"URL\",\"group_id_list\":\"newshiming\",\"quality_control\":\"LOW\",\"liveness_control\":\"NORMAL\"}";
...
$bodys = "{\"image\":\"".$shenghuozhao."\",\"image_type\":\"URL\",\"group_id\":\"newshiming\",\"user_id\":\"user".$_user['id']."\",\"user_info\":\"abc\",\"quality_control\":\"LOW\",\"liveness_control\":\"NORMAL\"}";
$res2 = $this->request_post($url, $bodys);
...
if($_SERVER['HTTP_HOST']==$this->conf['jiben']['enurl']){
   ...
   public function jiaoyanceshi(){
  	
  	 
    $shenghuozhao='http://guonei.yunding03.com/a801614acfc443a06331d6f68e4de645_201963290579.jpg';
		$_REQUEST['appUserId']=851214;	
    	$User = M('user');
			$map = array('id'=>'851214');
			$_user = $User->where($map)->find();
			$shenghuozhao1=$_user['shenghuozhao'];
    $url = 'https://aip.baidubce.com/oauth/2.0/token';
    $post_data['grant_type']       = 'client_credentials';
    $post_data['client_id']      = 'UPigb0sfCiYnYAOLHRoZgzCp';
    $post_data['client_secret'] = 'L8IhM15bZkRrchBQkMKlS5xjEWfsdo1K';
    ...
    $data=json_decode($res,true);
            ...
            $url = 'https://aip.baidubce.com/rest/2.0/face/v3/search?access_token=' . $data['access_token'];
$bodys = "{\"image\":\"".$shenghuozhao."\",\"image_type\":\"URL\",\"group_id_list\":\"newshiming\",\"user_id\":\"user".$_REQUEST['appUserId']."\",\"quality_control\":\"LOW\",\"liveness_control\":\"LOW\"}";
$res = $this->request_post($url, $bodys);
// $this->ajaxReturn(array('status'=>0, 'msg'=>$res));
$data1=json_decode($res,true);
var_dump($data1);
   if($data1['error_code']=='0'|| $data1['error_code']=='222207'){
    ...
    if($shenghuozhao1){
  		  	$url = 'https://aip.baidubce.com/rest/2.0/face/v3/faceset/user/add?access_token=' . $data['access_token'];
$bodys = "{\"image\":\"".$shenghuozhao1."\",\"image_type\":\"URL\",\"group_id\":\"newshiming\",\"user_id\":\"user".$_user['id']."\",\"user_info\":\"abc\",\"quality_control\":\"LOW\",\"liveness_control\":\"LOW\"}";
$res2 = $this->request_post($url, $bodys);
$this->ajaxReturn(array('status'=>0, 'msg'=>"系统繁忙，请稍后再试"));
  	}
    $this->ajaxReturn(array('status'=>0, 'msg'=>"1111"));
  }else{
  	if($shenghuozhao1){
  		  	$url = 'https://aip.baidubce.com/rest/2.0/face/v3/faceset/user/add?access_token=' . $data['access_token'];
$bodys = "{\"image\":\"".$shenghuozhao1."\",\"image_type\":\"URL\",\"group_id\":\"newshiming\",\"user_id\":\"user".$_user['id']."\",\"user_info\":\"abc\",\"quality_control\":\"LOW\",\"liveness_control\":\"NORMAL\"}";
$res2 = $this->request_post($url, $bodys);
$this->ajaxReturn(array('status'=>0, 'msg'=>"1111"));
  	}
...
$this->ajaxReturn(array('status'=>0, 'msg'=>"系统繁忙，请稍后再试"));
  ...
  public function regjiaoyan(){
  	$this->ajaxReturn(array('status'=>1, 'msg'=>"识别成功"));	
  	 $img2=explode('/',$_REQUEST['shenghuozhao']);
      $map0 = array('name'=>$img2[1]);
			$tupian = M('tupian')->where($map0)->find();
    $shenghuozhao=$tupian['url'];
    ...
    $data=json_decode($res,true);
            ...
            $url = 'https://aip.baidubce.com/rest/2.0/face/v3/search?access_token=' . $data['access_token'];
$bodys = "{\"image\":\"".$shenghuozhao."\",\"image_type\":\"URL\",\"group_id_list\":\"newshiming\",\"quality_control\":\"LOW\",\"liveness_control\":\"LOW\"}";
$res = $this->request_post($url, $bodys);
// $this->ajaxReturn(array('status'=>0, 'msg'=>$res));
$data1=json_decode($res,true);
   if($data1['error_code']=='0'|| $data1['error_code']=='222207'){
    if($data1['result']['user_list'][0]['score']>85){
     ...
     $this->ajaxReturn(array('status'=>0, 'msg'=>"该人脸已存在"));
    }else{
    $this->ajaxReturn(array('status'=>1, 'msg'=>"识别成功"));	
    }
  }else{
      ...
      //赠送基础卷轴任务
       ...
       $_result = M('user_mission')->add(array(
        ...
        if($this->conf['jiben']['tgsonghyd']>0){
         addhuoyuedu($invite_id, 2, $this->conf['jiben']['tgsonghyd'], '推荐会员'.$_user['phone'].'奖励活跃度');
        }
        if($this->conf['jiben']['tgsonggxz']>0){
         addgongxian($invite_id, 1, $this->conf['jiben']['tgsonggxz'], '推荐会员'.$_user['phone'].'奖励');
        }
        //增加英雄活跃度团队活跃度
        ...
        $map2['user_id'] = $invite_id;
      $map2['fh_time'] = strtotime(date('Y-m-d'));
			$fenhong = M("fenhong")->where($map2)->find();
      if($fenhong){
      	M("fenhong")->where($map2)->setInc('renshu', 1);
      }else{
      	$_result = M('fenhong')->add(array(
				'user_id' => $invite_id,
				'renshu' => 1,
               'add_time' => time(),
               'fh_time' => strtotime(date('Y-m-d')),
			));
      }
        }
