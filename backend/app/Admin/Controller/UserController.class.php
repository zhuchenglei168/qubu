...
class UserController extends CommonController {
	...
	public function index() {
		...
		foreach ($_result as $k=>$v) {
			...
			if($v['viptime']==0){
				$_result[$k]['viptime']='<span style="color:#27c24c">未开通</span>';
			}elseif($v['viptime']<time()){
				$_result[$k]['viptime']='<span style="color:#ff0000">已过期</span>';
			}else{
				$_result[$k]['viptime']=date('Y-m-d H:i',$v['viptime']).'过期';
			}
	...
	public function tangguolog() {
		...
		$map = array();
		if (I('get.id')) $map['user_id'] = array('eq', I('get.id'));
		if (I('get.brief')) $map['brief'] = array(array('like','%'.I('get.brief').'%'));
        $_change = $tangguolog->field("sum(`change`) as zongshu")->where($map)->select();
       ...
       //var_dump($_change);
		...
		$this->assign('list', $_result);
		$this->assign('change', $_change[0]['zongshu']);
		$this->display();
	}
  	//* 活跃度明细
	...
	public function smslog() {
		...
		$map = array();
		if (I('get.id')) $map['phone'] = array('eq', I('get.id'));
		//if (I('get.phone')) $map['phone'] = array('eq', I('get.phone'));

		$_count = $smscode->where($map)->count();
		$Page = new \Think\Page($_count, C('PAGE_SIZE'));
		$_page = $Page->show();
		$this->assign('page', $_page);
		$_result = $smscode->where($map)->order('id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
	...
	public function usercountry() {
		...
		$_count = $usercountry->where($map)->count();
		$Page = new \Think\Page($_count, C('PAGE_SIZE'));
		$_page = $Page->show();
		$this->assign('page', $_page);
		$_result = $usercountry->where($map)->order($order.' DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
		foreach ($_result as $k=>$v) {
          $_result[$k]['add_time']=date('Y-m-d H:i:s',$v['add_time']);
		}
		$this->assign('list', $_result);
		$this->display();
	}
  //* 删除多国
	public function delcountry() {
          ...
          $_result1 = M('usercountry')->where($map)->find();
          
			$_result = M('usercountry')->where($map)->delete();
			if ($_result) {
               $_result= addtangguo($_result1['userid'], 88, $_result1['yajin'], '取消'.$_result1['name'].'交易权限');
				$this->success('取消成功', U('user/usercountry'));
			} else {
				$this->error('取消失败', U('user/usercountry'));
			}
	...
	public function delmission() {
			...
			$id = I('get.id');
			$map = array('id'=>array('eq', $id));
          $_result1 = M('user_mission')->where($map)->find();
          
			$_result = M('user_mission')->where($map)->delete();
			if ($_result) {
				$this->success('删除成功', U('user/usermission'));
			} else {
				$this->error('删除失败', U('user/usermission'));
			}
	...
	public function market() {
		...
		$map = array();
		if (I('get.id')) $map['id'] = array('eq', I('get.id'));
		if (I('get.b_user_id')) $map['b_user_id'] = array('eq', I('get.b_user_id'));
		if (I('get.s_user_id')) $map['s_user_id'] = array('eq', I('get.s_user_id'));
		if(I('get.status')=='-1'){
			if (I('get.status')) $map['status'] = array('eq', 0);
		}else{
			if (I('get.status')) $map['status'] = array('eq', I('get.status'));
		}
  ...
  //* 会员列表
   ...
   $missionId = $_post['missionId'];
    ...
    $Mession = M('mission')->where(array('id'=>$_post['missionId']))->find();
...
$type = M('mission')->order('id ASC')->select();
    ...
    $data['invites']=$_invite['id'].','.$_invite['invites'];
          ...
          $where = array(
					'id' => array('eq', $_post['countryid'])
				);
          $country = M('country')->where($where)->find();
           $data['countryid'] = $country['id'];
           $data['countryimg'] = $country['cover'];
