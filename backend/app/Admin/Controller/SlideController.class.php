...
//* 超级VIP
	public function cjvip() {
		$cjvip = M('cjvip');
		$_count = $cjvip->count();
		$Page = new \Think\Page($_count, C('PAGE_SIZE'));
		$_page = $Page->show();
		$this->assign('page', $_page);
		$_result = $cjvip->order('id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
		$this->assign('list', $_result);
		$this->display();
	}
//* 国家列表
	public function duoguo() {
		...
		foreach($_result as $k => $v){
			$_result[$k]['renshu']= M('user')->where('countryid='.$v['id'])->count();
		}
		$this->assign('list', $_result);
		$this->display();
	}
...
//* 添加超级VIP
	public function addcjvip() {
		if (IS_POST) {
			$_post = I('post.');
			$data = array();
			$_post['name'] ? $data['name'] = $_post['name'] : $this->ajaxReturn(array('status'=>2, 'msg'=>'名称不能为空'));
			$data['yajin'] = $_post['yajin'];
			$data['day'] = $_post['day'];

			$_result = M('cjvip')->add($data);
			if ($_result) {
				$this->ajaxReturn(array('status'=>1, 'msg'=>'添加成功', 'url'=>U('slide/cjvip')));
			} else {
				$this->ajaxReturn(array('status'=>2, 'msg'=>'添加失败'));
			}
		} else {
	...
	public function editcountry() {
		if (IS_POST) {
          ...
          $data['yajin'] = $_post['yajin'];
			...
			if ($_result) {
				 $data1['huilv']=$_post['huilv'];
              $data1['name']=$_post['name'];
              $map = array('countryid'=>array('eq', I('get.id')));
               M('usercountry')->where($map)->save($data1);
				$this->ajaxReturn(array('status'=>1, 'msg'=>'编辑成功'));
			} else {
				$this->ajaxReturn(array('status'=>2, 'msg'=>'编辑失败'));
			}
