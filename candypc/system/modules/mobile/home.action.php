...
_messagemobile("兑换数量不能为空");
}
      if(empty($_POST['password'])){
_messagemobile("交换密码不能为空");
}
$token=$member['wxid'];
...
$member_money=$this->db->query("update `@#_member` set money=money+'$access_token[money]' where `uid`='$member[uid]'");
  $time=time();
				$member_money1=$this->db->query("insert into `@#_member_account` (`uid`,`type`,`pay`,`content`,`money`,`time`) values ('$member[uid]','1','账户','糖果兑换夺宝币','$access_token[money]','$time')");
				if($member_money){
				  _messagemobile("兑换成功！");exit;
				}else{
				  _messagemobile("兑换失败！");exit;
				}
}else{
 _messagemobile("兑换失败！");exit;
}
	...
	public function useraddress(){
		...
		if($_POST['submit']){
			...
			if(empty($jiedao)){
				_messagemobile("详细地址不能为空",WEB_PATH."/mobile/home/address");
			}
	...
	public function zhuanzhang(){
      ...
      if($_POST['submit1']){
			...
			$this->db->Autocommit_start();
				...
				$up_q3= $this->db->Query("INSERT INTO `@#_member_account`  SET `uid`= $uid, `type` = -1, `pay`= '账户', `content`= '给用户{$to_info['username']}转账{$cash}夺宝币', `money` = $cash ,`time` = $t");
				$up_q4= $this->db->Query("INSERT INTO `@#_member_account`  SET `uid`= {$to_info['uid']}, `type` = -1, `pay`= '账户', `content`= '获得用户{$member['username']}转账{$cash}夺宝币', `money` = $cash ,`time` = $t");
