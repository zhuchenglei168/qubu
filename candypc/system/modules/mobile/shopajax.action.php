...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserBuyList(){
	   ...
	   if($state==-1){
	     //参与云购的商品 全部...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.money!=b.yunjiage GROUP BY shopid ");


		  $shoplistall['listItems']=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.money!=b.yunjiage GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );
		}elseif($state==1){
		  //参与云购的商品 进行中...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.money!=b.yunjiage and b.q_end_time is null GROUP BY shopid  " );


		  $shoplistall['listItems']=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.money!=b.yunjiage and b.q_end_time is null GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );
		}else{
		  //参与云购的商品 已揭晓...
		  $shoplist=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.money!=b.yunjiage and b.q_end_time is not null GROUP BY shopid " );


		  $shoplistall['listItems']=$this->db->GetList("select *,sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$member[uid]' and b.money!=b.yunjiage and b.q_end_time is not null GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );
		}
