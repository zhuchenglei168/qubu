...
class index extends base {
	...
	public function init(){		
		...
		//他们正在云购	
		$go_record=$this->db->GetList("SELECT m.`uid`,m.`username`,m.`email`,m.`mobile`,m.`img`,mg.`gonumber`, mg.`shopname`, mg.`time`,s.`zongrenshu`,s.`canyurenshu` FROM `@#_member_go_record` AS mg LEFT JOIN `@#_member` AS m ON m.`uid` = mg.`uid` LEFT JOIN `@#_shoplist` AS s  ON  s.`id` = mg.`shopid` WHERE mg.`status` LIKE '%已付款%'  ORDER BY mg.`time` DESC LIMIT 0,5");
		//最新揭晓
		$shopqishu = $this->db->GetList("select id,sid,thumb,title, money,q_uid,q_user from `@#_shoplist` where `q_end_time` is not null and `q_showtime` = 'N' ORDER BY `q_end_time` DESC LIMIT 3");
	...
	public function item(){
		...
		if(!$itemlist[0]['q_uid']){
			if($itemlist[0]['id'] == $item['id'])
				$loopqishu.='<li><a href="'.WEB_PATH.'/goods/'.$itemlist[0]['id'].'"><b class="period_Ongoing period_ArrowCur bg_red" style="padding-left:0px;">'."第".$itemlist[0]['qishu']."期<i></i></b></a></li>";
			else
				$loopqishu.='<li><a class="gray02" href="'.WEB_PATH.'/goods/'.$itemlist[0]['id'].'"><b class="">'."第".$itemlist[0]['qishu']."期<i></i></b></a></li>";
		}else{		
			if($itemlist[0]['id'] == $item['id'])
				$loopqishu.='<li><a class="gray02" href="'.WEB_PATH.'/goods/'.$itemlist[0]['id'].'"><b class="">'."第".$itemlist[0]['qishu']."期<i></i></b></a></li>";
			else
				$loopqishu.='<li><a class="gray02" href="'.WEB_PATH.'/dataserver/'.$itemlist[0]['id'].'">第'.$itemlist[0]['qishu'].'期</a></li>';
		}
		unset($itemlist[0]);			
		foreach($itemlist as $key=>$qitem){
			if($key%9==0){
				$loopqishu.='</ul><ul class="Period_list bb_gray">';
			}
			if($qitem['id'] == $item['id'])
				$loopqishu.='<li><b class="period_Ongoing period_ArrowCur bg_red" style="padding-left:0px;">第'.$qitem['qishu'].'期</b></li>';
			else
				$loopqishu.='<li><a href="'.WEB_PATH.'/dataserver/'.$qitem['id'].'" class="gray02"><b class="">第'.$qitem['qishu'].'期<i></i></b></a></li>';	
		}
		$loopqishu.='</ul>';
	...
	public function dataserver(){	
		...
		foreach($itemlist as $key=>$qitem){			
			...
			if($qitem['id'] == $itemid){
				$loopqishu.='<li><b class="period_ArrowCur bg_red">第'.$qitem['qishu'].'期</b></li>';
			}else{
