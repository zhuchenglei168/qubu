...
class wapalipay_url extends SystemAction {
	...
	public function houtai(){
		...
		if( ! empty($doc->getElementsByTagName( "notify" )->item(0)->nodeValue) ) {
          ...
          //实际得到的支付金额
          $shiji=floor((100*$c_money)/$bili['value'])*0.01;
			$up_q2 = $this->db->Query("UPDATE `@#_member` SET `money` = `money` + $shiji where (`uid` = '$uid')");				
			$up_q3 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '支付宝充值', '$shiji', '$time')");
