...
class mobile extends base {
	...
	public function init(){
		...
		//人气商品
		$shoplistrenqi=$this->db->GetList("select * from `@#_shoplist` where `renqi`='1' and `q_end_time` is null ORDER BY id DESC LIMIT 100");
		//获取广告
		...
		//最新揭晓
		$shopqishu=$this->db->GetList("select * from `@#_shoplist` where `q_end_time` !='' and `money`!=`yunjiage` ORDER BY `q_end_time` DESC LIMIT 3");
...
//ajax获取商品列表信息
	...
	public function item(){
		...
		if(!$item)_messagemobile("Product does not exist!");
		if($item['q_end_time']){
		...
		//期数显示
		$loopqishu='';
		$loopqishu.='<li class="cur"><a href="javascript:;">'."Phase ".$item['qishu']."</a><b></b></li>";

		if(empty($itemlist)){
		foreach($itemlist as $qitem){
			$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/item/'.$qitem['id'].'" class="">Phase '.$qitem['qishu'].'</a></li>';
		...
		foreach($itemlist as $qitem){
			if($qitem['id'] == $itemid){
				...
				$loopqishu.='<li class="cur"><a href="javascript:;">'."Phase ".$itemlist[0]['qishu']."</a><b></b></li>";
			}else{
				$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/dataserver/'.$qitem['id'].'" >Phase '.$qitem['qishu'].'</a></li>';
			}
		}
		$gorecode=array();
		if(!empty($itemlist)){
...
if($us){
	...
	public function itemajax(){
		...
		echo '<div style="width:90%; margin-left:5%;"><img width="90%" src='.$temp['pic'].'></div><div class="txt"><h6>'.$temp['title'].'</h6><div class="zj"><span>Win the prize</span><span style="color: #5bb6ea;">'.$temp['user']."</span></div></div>";exit;
	...
	public function jf_item(){
		...
		if(empty($itemlist)){
		foreach($itemlist as $qitem){
			$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/jf_item/'.$qitem['id'].'" class="">Phase '.$qitem['qishu'].'</a></li>';
	...
	public function dataserver(){
		...
		if(empty($item['q_user_code'])){
			_messagemobile("This product is in progress");
		}
		...
		if(empty($itemlist[0]['q_end_time'])){
			$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemlist[0]['id'].'">'."Phase ".$itemlist[0]['qishu']."</a><b></b></li>";
		...
		foreach($itemlist as $qitem){
			if($qitem['id'] == $itemid){
				...
				$loopqishu.='<li class="cur"><a href="javascript:;">'."Phase ".$qitem['qishu']."</a><b></b></li>";
			}else{
				$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/dataserver/'.$qitem['id'].'" >Phase '.$qitem['qishu'].'</a></li>';
			}
		}
