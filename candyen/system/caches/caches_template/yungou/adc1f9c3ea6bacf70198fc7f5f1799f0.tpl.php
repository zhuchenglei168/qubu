    ...
    <section class="clearfix g-pay-lst">
		<ul>
		 <?php $ln=1; if(is_array($shoplist)) foreach($shoplist AS $key => $val): ?>
			...
			<li>
			    <a href="<?php echo WEB_PATH; ?>/mobile/mobile/item/<?php echo $val['id']; ?>" class="gray6">(Phase <?php echo $val['qishu']; ?>)<?php echo $val['title']; ?>  (<?php echo $val['title2']; ?>)</a>
			    <span>
			        <em class="orange arial"><?php echo $val['cart_xiaoji']; ?></em> person times
			    </span>
			</li>
		 <?php  endforeach; $ln++; unset($ln); ?>
		
		</ul>
		<p class="g-pay-Total gray9">Total：<span class="orange arial Fb F16"><?php echo $MoenyCount; ?></span> money</p>
    ...
    <section class="clearfix g-Cart">
	    <article class="clearfix m-round g-pay-ment">
			...
			<?php if($fufen_dikou >= $MoenyCount): ?>
				...
				<span>You can pay with a points（Your points：<?php echo $member['score']; ?>）</span>
				</li>
			<?php  else: ?>
			     <li class="gray6 z-pay-ff z-pay-grayC" style="display:none">
				<span>You don't have enough points（Your points：<?php echo $member['score']; ?>）</span>
				</li>
			<?php endif; ?>
			...
			<?php if($Money >= $MoenyCount): ?>			
				...
				<span>You can pay in money（Account surplus：<?php echo $Money; ?> ）</span>
				</li>
			<?php  else: ?>
			    <li class="gray6 z-pay-ye z-pay-grayC">
				<a href="<?php echo WEB_PATH; ?>/mobile/home/userrecharge" class="z-pay-Recharge">Recharge</a>
				<span>You don't have enough money（Account surplus：<?php echo $Money; ?> ）</span>
				</li> 
			<?php endif; ?>
		    </ul>
	    </article>
	    <article id="bankList" class="clearfix mt10 m-round g-pay-ment g-bank-ct" style="display:none">
		    <ul>	
		    	<li class="gray6 z-pay-grayC"><s class="z-arrow"></s>Choose payment method</li> 
	    ...
	    <div class="g-Total-bt">
		    <a id="btnPay" href="javascript:;" class="orgBtn">Confirm payment</a>
	    </div>
    </section>
